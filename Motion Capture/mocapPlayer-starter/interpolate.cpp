...
int main(int argc, char **argv)
{
	if (argc != 7 && argc != 8)
	{
		...
		printf("Usage: %s <input skeleton file> <input motion capture file> <interpolation type> <angle representation for interpolation> <N> <output motion capture file> <T>\n", argv[0]);
		...
		printf("  T: tolerance for non-uniform keyframes between +t or -t. Max is 5 (if not specified, 0 is applied)\n");
	...
	char * TString = (argc == 8) ? argv[7] : (char*)"0";
	...
	int T = strtol(TString, NULL, 10);
	if (T < 0) {
		printf("Error: invalid T value (%d). T must be positive\n", N);
		exit(1);
	}
	else if(T > N) {
		printf("Error: invalid T value (%d). T cannot > N (%d) \n", T, N);
		exit(1);
	}
	printf("T=%d\n", T);
	...
	interpolator.Interpolate(pInputMotion, &pOutputMotion, N, T);
