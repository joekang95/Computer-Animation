...
void resetScene_callback(Fl_Button *button, void *)
{
	...
	for (int i = 0; i < dropdown_counter; i++) {
		dropdown->remove(0);
	}
	dropdown_counter = 0;
}

void resetCamera_callback(Fl_Button *button, void *)
{
	camera.zoom = 1;
	camera.tw = 0;
	camera.el = -15;
	...
	camera.aty = 0;
	camera.atz = 0;
	glScalef(1.0f / scale_size, 1.0f / scale_size, 1.0f / scale_size);
	scale_size = 1.0f;
}
...
void dropdown_inc() {
	int subnum;
	subnum = displayer.GetNumSkeletons();

	char s[20];
	sprintf(s, "%d", dropdown_counter);
	dropdown->add(s);
	dropdown_counter++;
}
...
void skeletonID_callback(Fl_Value_Input *obj, void*)
{
	...
	if (subnum < 0)
	{
		...
		{
			dropdown->value(displayer.GetNumSkeletons() - 1);
	...
	case FL_DRAG:
		...
		else if (mouse.button == 2)
		{
			if (abs(delta_y) > abs(delta_x))
			{
				...
				scale_size *= float(1 + delta_y / 100.0);
