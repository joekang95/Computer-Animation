...
int sprite = 0, saveScreenToFile = 0, lbs = 1,  dls = 1;
...
void saveScreenshot(int windowWidth, int windowHeight, char *filename)
{
	...
	if (ppm_write(filename, in))
		printf("File saved Successfully to model folder\n");
	else
...
static void updateSkinnedMesh() {
	...
	skinning->applySkinning(fk->getJointSkinTransforms(), newPosv, fk->getNumJoints(), lbs);
...
static void idleFunction() {
	...
	ik->doIK(IKJointPos.data(), fk->getJointEulerAngles(), lbs, dls);
...
static void keyboardFunc(unsigned char key, int x, int y) {
	...
	case 'z':
		lbs = 1 - lbs;
		if (lbs) { cout << "Skinning Mode: Linear Blend Skinning\n"; }
		else { cout << "Skinning Mode: Dual Quaternion Skinning\n"; }
		break;

	case 'c':
		dls = 1 - dls;
		if (dls) { cout << "IK Mode: Damped Least Square IK\n"; }
		else { cout << "IK Mode: PsuedoInverse IK\n"; }
		break;
